...
import qapps.datastore.local.hbase.TransactionStackImpl.ThreadLocalTransactionStack;
...
/**
 * For now each thread is going to have its own stack. This prevents users from
 * sharing a transaction across threads and also prevents users from reliably
 * sharing a transaction across requests that happen to be serviced by the same
 * thread. When we start allowing users to create threads we could change this
 * implementation to allow transactions to be shared across threads, but there's
 * little point in supporting this now.
 * 
 */
class TransactionStackImpl implements TransactionStack {
	...
	private TransactionData getTransactionData(Transaction txn) {
		...
		TransactionData data = txnDataMap.txnIdToTransactionData.get(txn
				.getId());
		if (data == null) {
	...
	/**
	 * A wrapper for a ThreadLocal<TransactionDataMap> that gives us flexibility
	 * in terms of the lifecycle of the ThreadLocal values. This just exists so
	 * that our production code can use a static member and our test code can
	 * use an instance member (it's easy to end up with flaky tests when your
	 * tests rely on static members because it's too easy to forget to clear
	 * them out).
	 */
	 ...
	 * Given a given Transaction in the list we can find: The Futures whose
	 * completion must be blocked on when committing or rolling back the
	 * Transaction can be found. The entities that have been put and deleted as
	 * part of the Transaction (used for post op callbacks).
	 */
