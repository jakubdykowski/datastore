...
/**
 * Describes an object that helps us decide which transaction to use when a user
 * does not explicitly tell us which transaction to use. This is an internal
 * interface - there should be no need to expose any of this to users.
 * 
 * Typical usage will be to push a transaction, fetch it at some later point via
 * {@link #peek()}, and then later on pop it. Transactions that are pushed are
 * required to be popped before the end of the request. We make no guarantees
 * about the availability of transactions that are pushed in one request and
 * accessed in another request, and if a transaction that is pushed in one
 * request does happen to be accessible in another request, we make no
 * guarantees about the validity of that transaction against the datastore. In
 * otherwords, if you forget or fail to pop a transaction before the request
 * returns, all bets are off.
 * 
 * For users who want to think of their open transactions as a list rather than
 * a stack, we do support the ability to pop a specific {@link Transaction}.
 * This allows users to do things like push one transaction, push a second
 * transaction, and then remove the first transaction even though it isn't at
 * the top of the stack.
 * 
 * The management of the transaction objects is intentionally kept separate from
 * the management of transaction state to allow for modifications to the above
 * functionality without impact to the apis we expose to users. In other words,
 * how we manage transaction state should have nothing to do with how we keep
 * track of which transaction to use when one is not explicitly provided.
 * 
